...
async function getLocation(ctx, locName, key, host) {
  const presets = { '海口': { lon: '110.3288', lat: '20.0310' }, '三亚': { lon: '109.5119', lat: '18.2528' }, '儋州': { lon: '109.5768', lat: '19.5209' }, '琼海': { lon: '110.4746', lat: '19.2584' }, '万宁': { lon: '110.3893', lat: '18.7953' }, '文昌': { lon: '110.7530', lat: '19.6129' }, '东方': { lon: '108.6536', lat: '19.1017' }, '五指山': { lon: '109.5169', lat: '18.7752' }, '陵水': { lon: '110.0372', lat: '18.5050' }, '保亭': { lon: '109.7026', lat: '18.6390' }, '屯昌': { lon: '110.1029', lat: '19.3638' }, '澄迈': { lon: '110.0073', lat: '19.7364' }, '临高': { lon: '109.6877', lat: '19.9084' }, '定安': { lon: '110.3593', lat: '19.6849' }, '乐东': { lon: '109.1717', lat: '18.7478' }, '昌江': { lon: '109.0556', lat: '19.2983' }, '白沙': { lon: '109.4515', lat: '19.2240' }, '琼中': { lon: '109.8335', lat: '18.9982' }, '北京': { lon: '116.4074', lat: '39.9042' }, '上海': { lon: '121.4737', lat: '31.2304' }, '广州': { lon: '113.2644', lat: '23.1291' }, '深圳': { lon: '114.0579', lat: '22.5431' }, '杭州': { lon: '120.1551', lat: '30.2741' }, '成都': { lon: '104.0657', lat: '30.6595' }, '重庆': { lon: '106.5049', lat: '29.5630' }, '武汉': { lon: '114.2986', lat: '30.5844' }, '西安': { lon: '108.9480', lat: '34.2632' }, '南京': { lon: '118.7674', lat: '32.0415' }, '天津': { lon: '117.2008', lat: '39.0842' }, '苏州': { lon: '120.5853', lat: '31.2989' }, '青岛': { lon: '120.3826', lat: '36.0671' }, '厦门': { lon: '118.0894', lat: '24.4798' }, '长沙': { lon: '112.9388', lat: '28.2282' }, '郑州': { lon: '113.6654', lat: '34.7579' }, '沈阳': { lon: '123.4315', lat: '41.8057' }, '大连': { lon: '121.6147', lat: '38.9140' }, '昆明': { lon: '102.8329', lat: '25.0406' }, '哈尔滨': { lon: '126.5350', lat: '45.8038' }, '济南': { lon: '117.0009', lat: '36.6758' }, '合肥': { lon: '117.2272', lat: '31.8206' }, '福州': { lon: '119.3062', lat: '26.0753' } };
...
function getWeatherIcon(code) {
  const map = { '100': 'sun.max.fill', '101': 'cloud.sun.fill', '102': 'cloud.fill', '103': 'cloud.sun.fill', '104': 'cloud.fill', '300': 'cloud.drizzle.fill', '301': 'cloud.drizzle.fill', '302': 'cloud.sun.rain.fill', '303': 'cloud.heavyrain.fill', '305': 'cloud.rain.fill', '306': 'cloud.rain.fill', '307': 'cloud.heavyrain.fill', '308': 'cloud.heavyrain.fill', '309': 'cloud.rain.fill', '310': 'cloud.heavyrain.fill', '311': 'cloud.heavyrain.fill', '312': 'cloud.heavyrain.fill', '313': 'cloud.bolt.rain.fill', '400': 'snowflake', '401': 'snowflake', '402': 'snowflake', '403': 'snowflake', '404': 'cloud.sleet.fill', '405': 'cloud.sleet.fill', '406': 'cloud.sleet.fill', '407': 'cloud.sleet.fill', '500': 'cloud.fog.fill', '501': 'cloud.fog.fill', '502': 'cloud.fog.fill', '503': 'cloud.fog.fill', '504': 'cloud.fog.fill', '507': 'cloud.fog.fill', '508': 'cloud.fog.fill', '800': 'wind', '801': 'wind', '802': 'wind', '803': 'wind', '804': 'wind' };
...
function renderSmall(now, city, BG, C_T, C_S) {
  ...
  return { type: 'widget', padding: 14, backgroundGradient: BG, children: [ { type: 'stack', direction: 'row', alignItems: 'center', children: [ { type: 'text', text: city, font: { size: 'caption1', weight: 'bold' }, textColor: C_T }, { type: 'spacer' }, { type: 'text', text: timeStr, font: { size: 'caption2' }, textColor: C_S } ] }, { type: 'spacer' }, { type: 'stack', direction: 'row', alignItems: 'center', gap: 10, children: [ { type: 'image', src: `sf-symbol:${icon}`, width: 40, height: 40, color }, { type: 'stack', direction: 'column', children: [ { type: 'text', text: `${now.temp}°`, font: { size: 'title2', weight: 'bold' }, textColor: C_T }, { type: 'text', text: now.text, font: { size: 'caption1' }, textColor: C_S } ] } ] } ] };
}
    ...
    children: [
      {
        type: 'stack', direction: 'row', alignItems: 'center', children: [
          { type: 'stack', direction: 'row', alignItems: 'center', gap: 6, children: [ { type: 'image', src: 'sf-symbol:location.fill', width: 14, height: 14, color: { light: '#FF3B30', dark: '#FF453A' } }, { type: 'text', text: city, font: { size: 'title3', weight: 'bold' }, textColor: C_T } ] },
          { type: 'spacer' },
          { type: 'text', text: `AQI ${air.aqi}`, font: { size: 'caption1', weight: 'semibold' }, textColor: air.color },
          { type: 'spacer', length: 6 },
          { type: 'text', text: timeStr, font: { size: 'caption2' }, textColor: C_S }
        ]
      },
      { type: 'spacer' },
      {
        type: 'stack', direction: 'row', alignItems: 'center', gap: 16, children: [
          ...
          { type: 'stack', direction: 'column', flex: 1, children: [ { type: 'text', text: `${now.temp}°C`, font: { size: 'largeTitle', weight: 'bold' }, textColor: C_T }, { type: 'text', text: now.text, font: { size: 'title3' }, textColor: C_T } ] },
          { type: 'stack', direction: 'row', alignItems: 'center', gap: 4, children: [ { type: 'text', text: air.category, font: { size: 'title3', weight: 'bold' }, textColor: air.color } ] }
        ]
      },
      { type: 'spacer' },
      {
...
function createCompactItem(icon, value, iconColor, C_T) {
  return {
    ...
    children: [
      ...
      { type: 'text', text: value, font: { size: 'title3', weight: 'semibold' }, textColor: C_T }
    ]
  };
}
...
function renderAccessoryCompact(now, city, family) {
  const icon = getWeatherIcon(now.icon);
  return { type: 'widget', children: [{ type: 'stack', direction: 'row', alignItems: 'center', gap: 6, children: [{ type: 'image', src: `sf-symbol:${icon}`, width: 24, height: 24, color: getWeatherColor(now.icon) }, { type: 'text', text: `${now.temp}° ${city.slice(0,4)}`, font: { size: family === 'accessoryInline' ? 'footnote' : 'subheadline' }, textColor: { light: '#000', dark: '#FFF' } }] }] };
}

function renderError(msg) {
  return { type: 'widget', padding: 16, backgroundColor: { light: '#FFFFFF', dark: '#000000' }, children: [{ type: 'stack', direction: 'column', alignItems: 'center', gap: 8, children: [{ type: 'image', src: 'sf-symbol:exclamationmark.triangle.fill', width: 32, height: 32, color: { light: '#FF3B30', dark: '#FF453A' } }, { type: 'text', text: msg, font: { size: 'body' }, textColor: { light: '#FF3B30', dark: '#FF453A' }, textAlign: 'center' }] }] };
}
